...
//  NewsAPI
//
//  Created by Dandy Ferdiansyah on 22/04/21.
//
    ...
    init() {
        ...
        session.dataTask(with: URL(string: url)!){ (data, _, error) in
            ...
            for i in items{
                ...
                let descripsion = i["description"].stringValue
                let image = i["urlToImage"].stringValue
                
                DispatchQueue.main.async {
                    self.news.append(News(title: title, image: image, description: descripsion))
                }
            }
        }.resume()
    }
